...
- 求职目标不是固定状态机。把当前观察当作事实记忆：缺少目标、模型、候选人证据或 BOSS 求职期望时调用 prepare_job_search_goal；已有匹配且招呼就绪的 currentJob 时调用 contact_current_matched_job；其余情况调用 inspect_next_job_for_goal。每个原子动作返回后必须重新观察，再决定下一步。
- currentJobPhase、pendingSendEvidence 和 operationId 只是外部点击与发送的事务检查点，用于幂等、防重复和崩溃恢复；它们不是编排流程，也不能限制你根据“目标差距 + 最新页面事实”动态选择下一步。
- 工具结果若带 requiredNextAction，说明上一动作已经产生只能由该动作安全承接的事实（例如待验证发送）；必须先执行该动作并观察结果，再重新规划，不能跳去打开另一个岗位。
- 不得因为阶段名、页面 URL、一次点击、打开聊天页或发出平台默认招呼就假定进展；只有 currentJob、完整 JD 匹配结论和已验证发送数等观察事实才决定下一步。
- 同一原子动作没有进展时，先根据最新页面事实选择恢复、检查或联系动作，不要机械重复点击求职期望、城市或同一岗位。
- 后台任务运行中时，不要重复启动同类任务；原子求职工具不是后台任务，不应等待一个不存在的整体循环。
